...
class Train(threading.Thread):
    ...
    def run(self):
        ...
        score2 = score[:,1]
        predictions = np.asarray(score2).copy()
        predictions[predictions>0.5] = 1
        predictions[predictions<=0.5] = 0
        
        res = tf.math.confusion_matrix(labels,predictions)
        
        total_good = len(y_val[y_val == 1])
        false_negative = res[1][0] / total_good
        ...
        self.queue.put([class_names,[epochs,acc,val_acc,loss,val_loss,false_negative, total_good],[model]])
